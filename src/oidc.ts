...
hono_oidc.get('/auth', async (c) => {
    try {
        let { results } = await c.env.DB.prepare(
          "SELECT * FROM oidc_client WHERE client_id = ?",
        )
        ...
        //save this request
        let code = await hash_sha_256(Math.random().toString())
        let oidc_state_hash = await hash_sha_256((c.req.query('state') as string)+code+Date.now().toString())
